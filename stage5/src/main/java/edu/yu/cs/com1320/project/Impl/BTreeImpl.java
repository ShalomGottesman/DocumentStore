...
public class BTreeImpl implements BTree<URI, Document> {
    ...
    private int entries; //number of key-value pairs in the B-tree
     ...
     */
    public BTreeImpl(DocumentIOImpl docIO1) {
    ...
    public Document get(URI key)
    {
        ...
        if(entry != null) {
        	Document entryVal = qualifyValue(entry, key); 
           return entryVal;
        }
        return null;
    }
    
    private Document qualifyValue(Entry entry, URI key) {
    	if (entry.val.getClass() == new File("").getClass()) {
    		entry.val = docIO.deserialize((URI) key);
    	}
     ...
     * @param key the key
     * @param val the value
     * @return old value before overwritten
     * @throws IllegalArgumentException if {@code key} is {@code null}
     */
    public Document put(URI key, Document val) {
        ...
        {
        	Document oldVal =  (Document) alreadyThere.val;
            alreadyThere.val = val;
            return oldVal;
        }

        Node newNode = this.put(this.root, key, val, this.height); // need to check exactly what this does to handle returns
        this.entries++;
        if (newNode == null)
        {
        ...
        this.height++;
        return null; //at this point, we already know the insert was a new put as seen in the second if statment, so return null as there is no old value
    }
     ...
     */
    private Node put(Node currentNode, URI key, Document val, int height)
    {
        int j;
    ...
    private static boolean less(URI k1, URI k2)
    {
        return k1.compareTo(k2) < 0;
    }

    private static boolean isEqual(URI k1, URI k2)
    {
        return k1.compareTo(k2) == 0;
    }

	@Override
	public void moveToDisk(URI key) throws Exception {
		if (key == null)
        {
            throw new IllegalArgumentException("argument to get() is null");
        }
        ...
        if(entry != null)
        {
            entry.val = docIO.serialize((Document) entry.val);
        }
        return;
	}
}